...
Matrix3d integrateImuData() {
    ...
    while (end_iter != imu_msg_buffer.end()) {
        if ((end_iter->header.stamp.toSec() - cur_image_time) < 0.005)
            ++end_iter;
        else
            break;
    }
...
void multi_callback(const sensor_msgs::ImageConstPtr& img_msg, const sensor_msgs::PointCloudConstPtr &edgedes_msg)
{
    if(first_image_flag)
    {
        first_image_flag = false;
        first_image_time = img_msg->header.stamp.toSec();
        cur_image_time = last_image_time = img_msg->header.stamp.toSec();
        return;
    }
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if (i != 1 || !STEREO_TRACK)     // 单目 或者 非双目,   #### readImage： 这个函数里 做了很多很多事, 提取特征 ####
        {
            ...
            trackerData[i].readImage(ptr->image.rowRange(ROW * i, ROW * (i + 1)),img_msg->header.stamp.toSec(),relative_R, edgedes_msg,init_flag);
        }
        else
        {
...
//                tmp_img = trackerData[0].cur_img;
                ...
                if (i != 1 || !STEREO_TRACK)
                {
                    ...
                    {
                        double len = std::min(1.0, 1.0 * trackerData[i].track_cnt[j] *3/ WINDOW_SIZE);
...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> subscriber_edgeimage(n,IMAGE_TOPIC,1000,ros::TransportHints().tcpNoDelay());
    message_filters::Subscriber<sensor_msgs::PointCloud> subscriber_edgedes(n,"/edgedes",1000,ros::TransportHints().tcpNoDelay());
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::PointCloud> syncPolicy;
    message_filters::Synchronizer<syncPolicy> sync(syncPolicy(10), subscriber_edgeimage, subscriber_edgedes);  
    sync.registerCallback(boost::bind(&multi_callback, _1, _2));
    ros::Subscriber sub_imu = n.subscribe(IMU_TOPIC, 1000, imu_callback);
    ros::Subscriber sub_data = n.subscribe("/ETIO/Intrinsic", 1000, data_callback);
