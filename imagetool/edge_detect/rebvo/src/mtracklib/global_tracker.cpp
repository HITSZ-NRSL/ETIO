                                  ...
                                  T &fi)                //Pixel residual
{
    ...
    if(f_kl.desValid&&kl.desValid)
    {
        float scores = 0;
        for(int i = 0;i<6;i++)
        {
            scores += f_kl.des[i]*kl.des[i];
        }
                                  ...
                                  T &fi)                //Pixel residual
{
    ...
    //descriptor:
    double bestscore = kl.des[0]*f_kl.des[0]+kl.des[1]*f_kl.des[1]+kl.des[2]*f_kl.des[2]+kl.des[3]*f_kl.des[3]+kl.des[4]*f_kl.des[4]+kl.des[5]*f_kl.des[5];    
    kl.m_id_f=field[f_inx].ikl;

    if(bestscore<0.7)
    {
        if(f_kl.p_id!=-1)
        {
            KeyLine &kl_1 = (*klist_f)[f_kl.p_id];
            double curscore = kl.des[0]*kl_1.des[0]+kl.des[1]*kl_1.des[1]+kl.des[2]*kl_1.des[2]+kl.des[3]*kl_1.des[3]+kl.des[4]*kl_1.des[4]+kl.des[5]*kl_1.des[5];
            ...
            if(kl_1.p_id!=-1)
            {
                KeyLine &kl_2 = (*klist_f)[kl_1.p_id];
                double curscore = kl.des[0]*kl_2.des[0]+kl.des[1]*kl_2.des[1]+kl.des[2]*kl_2.des[2]+kl.des[3]*kl_2.des[3]+kl.des[4]*kl_2.des[4]+kl.des[5]*kl_2.des[5];
                if(curscore>bestscore)
                {
                    bestscore = curscore;
                    kl.m_id_f=kl_1.p_id;
        ...
        if(f_kl.n_id!=-1)
        {
            KeyLine &kl_3 = (*klist_f)[f_kl.n_id];
            double curscore = kl.des[0]*kl_3.des[0]+kl.des[1]*kl_3.des[1]+kl.des[2]*kl_3.des[2]+kl.des[3]*kl_3.des[3]+kl.des[4]*kl_3.des[4]+kl.des[5]*kl_3.des[5];
            if(curscore>bestscore)
            {
                bestscore = curscore;
                kl.m_id_f=f_kl.n_id;
            ...
            if(kl_3.n_id!=-1)
            {
                KeyLine &kl_4 = (*klist_f)[kl_3.n_id];
                double curscore = kl.des[0]*kl_4.des[0]+kl.des[1]*kl_4.des[1]+kl.des[2]*kl_4.des[2]+kl.des[3]*kl_4.des[3]+kl.des[4]*kl_4.des[4]+kl.des[5]*kl_4.des[5];
                if(curscore>bestscore)
                {
                    bestscore = curscore;
                    kl.m_id_f=kl_3.n_id;
    ...
    KeyLine &f_kl1=(*klist_f)[kl.m_id_f]; //Field keyline

    T dx=p.x-f_kl1.c_p.x;
    T dy=p.y-f_kl1.c_p.y;

    fi=(dx*f_kl1.u_m.x+dy*f_kl1.u_m.y);   //Residual in the direction of the gradient

    df_dx=f_kl1.u_m.x;          //Return weigthed gradient
    df_dy=f_kl1.u_m.y;
...
template float global_tracker::Calc_f_J2(const int f_inx,float &df_dx,float &df_dy,KeyLine &kl,const Point2D<float> &p,const float &max_r, const float &simil_t,int &mnum,float &fi) ;
                                 ...
                                 T* DResidualNew)                   //New iteration Distance Residuals
{
    ...
    // std::cout<<"process Time:"<<processTime*1000<<"ms"<<std::endl;
